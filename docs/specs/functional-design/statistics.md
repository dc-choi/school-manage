...
| 그룹별 통계 총계 행 (로드맵 2단계) | 그룹별 통계 테이블 마지막에 전체 총계 표시 | 미구현 |
...
- 그룹별 상세 통계 테이블에 전체 합산(총계)이 없어 한눈에 파악이 어려움
- 각 그룹의 인원/출석률/평균 인원은 보이지만 전체 현황 파악을 위해 수동 합산 필요

### 사용자 플로우

1. 교리교사가 대시보드에서 그룹별 통계 테이블 확인
...
| 열 | 계산 방법 |
|------|----------|
| 인원 | 모든 그룹의 `totalStudents` 합계 |
| 출석률 | 프론트엔드에서 그룹별 데이터 기반 가중 평균 계산: `Σ(그룹 출석률 × 그룹 인원) / Σ(그룹 인원)` |
| 평균 출석 인원 | 모든 그룹의 `avgAttendance` 합계 |
...
| 상황 | 처리 방법 |
|------|----------|
| 그룹이 0개 | 총계 행 미표시 (기존 "데이터 없음" 유지) |
| 전체 인원 0명 | 출석률 0%, 평균 0명 |
| 일부 그룹 인원 0명 | 해당 그룹은 가중 평균에서 자연스럽게 제외 (인원 0 × 출석률 = 0) |

### 테스트 시나리오

**정상 케이스:**

1. 그룹 2개 데이터 → 총계 행에 합산된 인원, 가중 평균 출석률, 합산된 평균 출석 표시
2. 그룹 1개 데이터 → 총계 = 해당 그룹과 동일
