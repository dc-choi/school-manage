...
> 검증 체계, 6단계 문서 정리, 예외 처리, 비기능 워크플로우는 `rules/specs-lifecycle.md` 참조
...
| Development | 어떻게 구현하는가?  | 역할별 구현 명세, 테스트 시나리오 (rules 참조, 중복 금지) | rules 기준 정합? Task와 1:1 대응?         |
...
기능 설계 문서가 **190줄을 초과**하면 논리적 단위로 분리합니다.
...
| 섹션 | 작성 기준 | 생략 시 기본값 |
|------|----------|-------------|
| 권한/보안 | 기본(Bearer 토큰)과 다를 때 | `scopedProcedure` 인증 |
| 성능/제약 | 트래픽/데이터 규모가 "소규모" 초과 시 | 소규모 가정 |
| 측정/모니터링 | GA4 이벤트/알림이 있을 때 | 없음 |
