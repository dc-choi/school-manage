...
| 이달의 축일자 목록 (로드맵 2단계) | 대시보드에 이달 축일 멤버 카드 표시 | 미구현 |
...
- 교리교사가 이달에 축일이 있는 멤버를 파악하려면 명단을 일일이 확인해야 함
...
1. 교리교사가 대시보드에 진입
2. 이달에 축일이 있는 멤버가 있으면 "이달의 축일자" 카드 표시
3. 카드에 멤버 이름, 세례명, 축일 날짜(MM/DD) 표시
...
| 프로시저 | 타입 | 설명 |
|----------|------|------|
| `student.feastDayList` | query | 이달의 축일자 멤버 목록 조회 |
...
| 기능 | 동작 요약 |
|------|----------|
| 축일자 필터링 | `baptizedAt` 필드의 월 부분(MM)이 요청 월과 일치하는 멤버 조회 |
| 대상 범위 | 계정 소속 그룹의 재학생만 (deletedAt=null, graduatedAt=null) |
| 정렬 | `baptizedAt`의 DD 부분 오름차순 |
| 형식 불일치 | `baptizedAt`이 비어있거나 MM/DD 형식이 아닌 멤버는 제외 |

### 예외/엣지 케이스

| 상황 | 처리 방법 |
|------|----------|
| baptizedAt 비어있음 | 해당 멤버 제외 |
| baptizedAt 형식 불일치 | 해당 멤버 제외 |
...
1. 1월 조회 → baptizedAt이 "01/XX"인 멤버만 반환
2. 축일자 없는 월 조회 → 빈 배열
3. 결과 날짜순 정렬 확인 (01/05 → 01/15 → 01/29)

**예외 케이스:**

1. baptizedAt이 null인 멤버 → 결과에서 제외
2. baptizedAt이 "MM/DD" 형식이 아닌 멤버 → 결과에서 제외
3. 삭제/졸업 멤버 → 결과에서 제외
