...
| **Target Bugfix**         | -      | 2건 미착수 (P3 2건) + 14건 완료                                                                                                |
...
| P3       | Attendance 중복 레코드 방지                  | ✅ 완료  | `(student_id, date)` UNIQUE + Kysely `insertInto().onDuplicateKeyUpdate()` (atomic upsert). groupId historical 보존 (UPDATE 절 미포함). 통합 테스트 26건 (race retry + 트랜잭션 원자성 + groupId 보존). 마이그레이션 SQL: 단일 ALTER TABLE. 설계: `attendance-management.md` |
