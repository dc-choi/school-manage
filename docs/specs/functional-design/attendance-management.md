...
| 화면               | 주요 요소                                                                  |
| ------------------ | -------------------------------------------------------------------------- |
| 출석 현황 (테이블) | 학년/연도/정렬 select + 헤더 우상단 인디케이터, 학생(세로) × 날짜(가로) 셀 |
| 출석 달력          | 학년/월 선택, 날짜별 미사/교리 인원 분리 표시, 의무축일 표시               |
| 출석 입력 모달     | 미사/교리 체크박스, 자동 상태 계산, 즉시 큐 적재                           |
...
| 우수 출석 | 누적 점수 (◎=2, ○=1, △=1, 미입력=0) | 동점은 가나다 보조. 1~3위에 Trophy 배지 |

정렬 상태는 그룹/연도별 sessionStorage에 보관.
...
- `attendance_sort_clicked` (sort_type: 'registration' | 'name' | 'top_attendance')
