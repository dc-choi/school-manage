...
- **auth.checkId**: `name` → `available`(boolean). 판정 스코프는 활성+탈퇴 계정 전체 (DB UNIQUE와 동일 — 탈퇴 계정 name은 복원 예약 슬롯이라 사용 불가). 에러: 400 형식 오류
