...
- PRD: `docs/specs/prd/daily-stats-dashboard.md` (일간 출석 컬럼)
...
| 일간 출석 컬럼 (2단계)      | 학년별 통계 테이블에 일간 출석/출석률 + 통계 페이지 날짜 필터 | 구현 완료 |
...
| 성별 분포           | 남/여 출석 비율                                                                                                                                                                                        |
| 학년별 출석률 순위  | TOP 5                                                                                                                                                                                                  |
| 학년별 상세 통계    | 각 학년 일간/주간/월간/연간 출석률 + 평균 출석. 모바일(<768px)에서는 학년명 + 일간 출석 + 주간/월간 평균 출석만 표시. 일간은 통계 페이지 날짜 필터(`<input type="date">`) 또는 가장 최근 출석일 기본값 |
| 전체 우수 출석 학생 | TOP 5                                                                                                                                                                                                  |

> **전체 우수 출석 TOP 5 스코프 (S-1, 2026-06-01)**: "전체 우수 출석 학생" 카드는 연간 누적 점수 순위다. 대시보드/통계 페이지의 기간 필터(월/주)와 무관하게 **연도 스코프로 고정**한다 — 월/주를 적용하면 조회 범위가 단일 주로 좁혀져 점수 상한이 2점(◎ 1회)으로 붕괴되어 순위가 무의미해진다. 성별 분포/학년별 상세 통계는 기간 필터를 그대로 반영(의도된 동작).
...
| `statistics.groupStatistics` | 학년별 상세 통계 (일간 포함) | `effectiveDay`, groups[](daily/weekly/monthly/yearly). 입력에 `day?: YYYY-MM-DD` (미지정 시 가장 최근 출석일 → 오늘 폴백). daily는 `attendanceCount` + `attendanceRate` |
