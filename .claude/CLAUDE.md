...
프로젝트 소개와 ERD는 @README.md, 사용자(개발자) 컨텍스트는 @.claude/user.md 참조.
...
- **DB는 MySQL**이다. 어댑터 패키지 이름이 `@prisma/adapter-mariadb`이지만 실제 운영/로컬 DB는 MySQL — 인덱스/플래너/SQL 동작은 MySQL 기준으로 판단한다.
...
| `/biz` · `/bs` · `/bs-to-target` · `/biz-audit` | 사업 워크플로우 / 브레인스토밍 / TARGET 반영 / 사업 문서 교차 점검 |
| `/biz-pulse`                                    | DB 운영 메일 paste + GA4/Clarity MCP로 정량 스냅샷 (STATUS 보강)   |
...
- **"무료" 표현 사용 금지**: 프로덕트 코드, 콘텐츠 모두에서 "무료"를 쓰지 않는다. (예: "무료로 시작하기" → "바로 시작하기") 단, `docs/business/` 내부 사업 문서의 플랜 티어 명칭("무료 플랜")과 진입장벽 전략 서술은 예외 — 고객 노출 카피가 아닌 내부 분석 용어.
- **middle-dot(`·`, U+00B7) 사용 금지**: UI 문구·콘텐츠·문서 어디서든 항목 구분에 `·`을 쓰지 않는다. 사용자가 AI 작성물 느낌으로 인지하여 거부감. 대안: 공백 1~2칸, 줄바꿈, 슬래시(`/`).
- **가톨릭 카피는 "주일" 사용**: 사용자 가시 UI 카피(`apps/web/src/**`), 외부 노출 콘텐츠(`docs/content/**`, `README.md`, OG/랜딩)에서 "일요일" 대신 **"주일"**을 쓴다. 토요특전미사 때문에 "일요일"은 도메인적으로 부정확. 단, **코드/요일명/캘린더 헤더/테스트**(예: `packages/utils/src/{date,liturgical}.ts`, `apps/web/src/pages/attendance/CalendarGrid.tsx`, `*.test.ts`)는 "일요일" 그대로 보존. 내부 사업/스펙 문서는 별건 정리. 상세 → [content.md](rules/content.md).
