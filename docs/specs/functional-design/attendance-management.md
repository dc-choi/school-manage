...
| 화면 | 설명 | 주요 요소 |
|------|------|----------|
| 출석 현황 (테이블) | 학년별 출석 테이블 | 연도 선택, 학생 목록 (세로), 날짜 (가로), 출석 입력 셀 |
| 출석 달력 | 월별 달력 뷰 | 학년/월 선택, 날짜별 출석 현황, 의무축일 표시 |
...
| 프로시저 | 타입 | 설명 |
|----------|------|------|
| `group.attendance` | query | 학년 출석 현황 조회 (테이블 뷰용) |
...
- 변경사항은 즉시 저장되므로 학년 전환 시 별도 처리 불필요
...
1. 학년 출석 조회 → year, students, attendances 반환
