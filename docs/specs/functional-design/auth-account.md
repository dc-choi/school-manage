...
| 포함 | 제외 |
|------|------|
| AuthLayout 스플릿 레이아웃 (히어로 + 카드) | 별도 랜딩페이지 → **1단계 격상, 아래 섹션에서 별도 설계** |
...
① 히어로: 후킹 헤드라인 + 서브카피 + 스크롤 가이드
    ↓
② 공감: Pain Points 3건 ("종이 출석부", "명단 분실", "출석상")
    ↓
...
> 사업 근거: `docs/business/1_market/competitive-analysis.md` (가톨릭 전용 출석 관리 도구 시장 부재)
> 사업 근거: `docs/business/3_gtm/gtm.md` (포지셔닝 메시지)

#### 배경

- 현재 헤드라인("종이 출석부, 엑셀 정리 / 폰 하나로 바꿔보세요")은 범용 메시지 — 모든 출석 앱이 동일하게 쓸 수 있음
- 경쟁사 분석 결과: **가톨릭 교리교사/모임 운영자용 출석 관리 전용 도구가 시장에 존재하지 않음**
- "유일한 가톨릭 전용 도구"라는 검증된 차별점을 첫 화면에서 전달해야 함

#### 목표

- 방문자가 첫 화면에서 "가톨릭 전용 유일한 도구"임을 인지
- 5단 구조, 인터랙티브 데모, 레이아웃은 유지 (콘텐츠/메시지만 교체)

#### 변경 범위

| 섹션 | 변경 내용 | 비고 |
|------|----------|------|
| ① Hero | 헤드라인 + 서브카피 교체 | 포지셔닝 반영 |
| ② Pain Points | 변경 없음 | 현재 메시지 적절 |
| ③ Features | 변경 없음 | 현재 기능 카드 적절 |
| ④ Interactive Demo | 변경 없음 | |
| ⑤ CTA | 사회적 증거 메시지 간소화 | 중복 제거 |
| Footer | 서비스명 업데이트 | |
...
| 요소 | 현재 | 변경 후 |
|------|------|--------|
| h1 | "종이 출석부, 엑셀 정리 / 폰 하나로 바꿔보세요" | "주일학교 교리교사를 위한 / 출석·멤버 관리 도구" |
| 서브카피 | "출석 체크부터 통계까지, 한곳에서 다 돼요." | "엑셀 대신, 한곳에서. 달력에서 탭 한 번이면 출석 완료." |

- 근거: GTM 교리교사 메시지 기반, 1차 타겟(주일학교 교리교사)을 직접 명시
- "가톨릭 모임"은 추상적 → 구체적 타겟으로 교체

**⑤ CTA**

| 요소 | 현재 | 변경 후 |
|------|------|--------|
| 사회적 증거 | "{N}개 단체가 이미 폰으로 출석 관리 중이에요" | "{N}개 단체가 이미 사용하고 있어요" |
| 안내 문구 2행 | "이번 주일부터 폰 하나로 출석 관리해보세요." | "이번 주일부터 써보세요." |

- 근거: Hero에서 포지셔닝 전달 완료 → CTA는 행동 유도에 집중, "폰으로 출석 관리" 반복 제거

**Footer**

| 요소 | 현재 | 변경 후 |
|------|------|--------|
| 카피라이트 | "© 2022 주일학교 관리 프로그램" | "© 2022 주일학교 출석부" |

#### 테스트 시나리오

1. **TC-LP1**: Hero 헤드라인에 "주일학교 교리교사" 타겟 명시 확인
2. **TC-LP2**: 기존 5단 구조 + 인터랙티브 데모 정상 동작 유지
3. **TC-LP3**: 모바일/데스크톱 반응형 레이아웃 유지 (break-keep + text-balance)
