...
- 목표 상태:
    ...
    - 21:00 일일 보고서 본문 상단에 "**{churchCount}개 본당에서 {accountCount}명의 선생님들이 {studentCount.toLocaleString('ko-KR')}명의 학생과 함께하고 있어요**" 형식의 사회적 증거 한 줄을 노출 (랜딩/AuthLayout 카피와 동일).
...
1. **일일 보고 수신**: 매일 21:00 KST에 운영자가 받은 메일 첫 줄에서 "{N}개 본당에서 {M}명의 선생님들이 {K}명의 학생과 함께하고 있어요"를 확인하고, 이어지는 조직 활동/계정 현황 표를 본다.
