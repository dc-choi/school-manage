...
| 8   | **STATUS 누적 섹션**     | STATUS.md에 `### 주요 변화 (X 갱신)` 섹션이 1건 이상 → **WARN** (제거 + 변화 서사는 커밋 메시지로). 0건이 정상         |
| 9   | **STATUS 메타 누적**     | STATUS.md 메타 라인의 `최종 업데이트:` 항목에 갱신 요약 2건 이상 누적 → **WARN** (1건으로 단축, 이력은 git log가 대체) |
...
이슈 N건:
  ...
  [WARN] STATUS.md 주요 변화 섹션 3개 누적 (05-05/05-04/05-03) — 제거 + 커밋 메시지로 이전 필요
