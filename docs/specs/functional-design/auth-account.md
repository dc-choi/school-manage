...
- 원인: "다음에 뭘 해야 하는지 모름" + "그룹/멤버 구조 미이해"
...
대시보드: 온보딩 체크리스트 표시 (일반 대시보드 대신)
   ↓
① "그룹 만들기" 클릭 → /groups/new → 그룹 생성 → 대시보드 복귀
   ↓ (1단계 완료 ✅)
② "멤버 등록" 클릭 → /students/new → 멤버 등록 → 대시보드 복귀
   ↓ (2단계 완료 ✅)
...
| 단계 | 완료 조건 | 사용 API |
|------|----------|----------|
| ① 그룹 만들기 | 그룹 1개 이상 존재 | `group.list` |
| ② 멤버 등록 | 멤버 1명 이상 존재 | `student.list` |
...
| 페이지 | 현재 빈 상태 메시지 | 개선 |
|--------|-------------------|------|
| 그룹 목록 (`/groups`) | "등록된 그룹이 없습니다." | + "그룹을 만들면 멤버를 등록할 수 있어요" |
| 멤버 목록 (`/students`) | "등록된 멤버가 없습니다." | + "멤버를 등록하면 출석 체크를 시작할 수 있어요" |
...
| 프로시저 | 용도 |
|----------|------|
| `group.list` | 그룹 존재 여부 판단 |
| `student.list` | 멤버 존재 여부 판단 |
...
| 상황 | 처리 방법 |
|------|----------|
| 그룹 있지만 멤버 0명 | 2단계 진행 중 (체크리스트 표시) |
| 멤버 있지만 출석 0건 | 3단계 진행 중 (체크리스트 표시) |
...
1. **TC-OB1**: 그룹/멤버/출석 0건 사용자 → 대시보드에 체크리스트 표시, 1단계 강조
2. **TC-OB2**: 그룹 1개 이상, 멤버 0명 → 1단계 완료, 2단계 강조
3. **TC-OB3**: 그룹+멤버 있음, 출석 0건 → 1·2단계 완료, 3단계 강조
