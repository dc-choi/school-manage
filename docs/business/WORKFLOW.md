...
브레인스토밍(`docs/brainstorm/`)의 결과물은 **사용자가 결정하기 전까지 사업 문서(로드맵, STATUS 등)에 반영하지 않는다.**

| 브레인스토밍 결과 | 사업 문서 반영 조건 |
|--------------|---------------|
...
> **핵심 원칙**: 브레인스토밍은 리서치이지 결정이 아니다. 사용자의 명시적 결정 없이 사업 문서를 변경하지 않는다.
