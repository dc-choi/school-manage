...
### 로드맵 1단계 (무료 핵심 완성) — 전체 구현 완료

> 참고: `docs/business/6_roadmap/roadmap.md`

- [x] 대시보드 통계 (주간/월간 출석률, 성별 분포, 그룹별 순위, 우수 출석 TOP 5)
- [x] 출석부 자동 저장 + 달력 UI (날짜 클릭 모달, 미사/교리 체크, 의무축일 표시)
- [x] 일괄 졸업 처리 (소프트 처리, 졸업생 필터)
- [x] 그룹 관리 UI/UX 개선 (상세 페이지, 일괄 삭제)
- [x] 학생 관리 UI/UX 개선 (상세 페이지, 일괄 삭제/복구, 삭제 필터)
- [x] 회원가입 (ID 중복 확인, 자동 로그인)
- [x] 목록 페이지네이션 상태 유지 (`?page=N`, 뒤로가기 보존)
...
> 현재 보류 항목 없음 (학생 엑셀 Import → `prd/student-excel-import.md`로 분리, TARGET 등록)
