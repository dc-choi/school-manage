...
| 이달의 축일자 목록 (로드맵 2단계) | 대시보드에 이달 축일 학생 카드 표시 | 미구현 |
...
**기본 플로우:**
1. 사용자가 학생 명단 화면 진입
2. 계정 소속 학년의 전체 학생 목록 조회 (재학생 필터 기본)
...
| 학생 추가 | 신규 학생 등록 | 정보 입력 폼, 학년 선택, 저장 버튼 |
...
| 검색 필터 | 이름, 세례명, 축일로 검색 |
| 테이블 헤더 | 선택 체크박스, 이름, 세례명, 나이, 학년, 연락처 |
...
| 기능 | 동작 요약 |
|------|----------|
| 학생 목록 | accountId 소속 학년의 학생 조회, 삭제 필터/검색/페이지네이션 적용 |
...
| 진급 (promote) | 초등부: 학년별 학년 이동 (age>=8), 중고등부: 19세→고3, 20세→성인 |
...
- 교리교사가 이달에 축일이 있는 학생을 파악하려면 명단을 일일이 확인해야 함
- Student 테이블에 `baptizedAt` 필드(MM/DD 형식)가 이미 존재
- 대시보드 진입 시 이달의 축일자를 자동 표시하여 행정 부담 감소

### 사용자 플로우

1. 교리교사가 대시보드에 진입
2. 이달에 축일이 있는 학생이 있으면 "이달의 축일자" 카드 표시
3. 카드에 학생 이름, 세례명, 축일 날짜(MM/DD) 표시
4. 축일자가 없으면 "이달의 축일자가 없습니다" 안내 문구 표시
...
|------|------|
| 카드 제목 | "이달의 축일자" |
| 학생 항목 | 이름, 세례명, 축일 날짜 (MM/DD) |
| 정렬 | 날짜순 (이달 내 가까운 축일 먼저) |
| 빈 상태 | 축일자가 없으면 "이달의 축일자가 없습니다" 표시 |

### API

| 프로시저 | 타입 | 설명 |
|----------|------|------|
| `student.feastDayList` | query | 이달의 축일자 학생 목록 조회 |

| 프로시저 | 요청 필드 | 응답 필드 |
|----------|----------|----------|
| `student.feastDayList` | month(number, 1~12) | students(array: societyName, catholicName, baptizedAt, groupName) |

### 비즈니스 로직

| 기능 | 동작 요약 |
|------|----------|
| 축일자 필터링 | `baptizedAt` 필드의 월 부분(MM)이 요청 월과 일치하는 학생 조회 |
| 대상 범위 | 계정 소속 학년의 재학생만 (deletedAt=null, graduatedAt=null) |
| 정렬 | `baptizedAt`의 DD 부분 오름차순 |
| 형식 불일치 | `baptizedAt`이 비어있거나 MM/DD 형식이 아닌 학생은 제외 |

### 예외/엣지 케이스

| 상황 | 처리 방법 |
|------|----------|
| baptizedAt 비어있음 | 해당 학생 제외 |
| baptizedAt 형식 불일치 | 해당 학생 제외 |
| 이달 축일자 없음 | 빈 배열 반환, "이달의 축일자가 없습니다" 표시 |
| 토큰 누락 | 401 UNAUTHORIZED |

### 측정

- GA4 이벤트: `patron_feast_card_viewed` (축일자 카드 노출 시)

### 테스트 시나리오

**정상 케이스:**

1. 1월 조회 → baptizedAt이 "01/XX"인 학생만 반환
2. 축일자 없는 월 조회 → 빈 배열
3. 결과 날짜순 정렬 확인 (01/05 → 01/15 → 01/29)

**예외 케이스:**

1. baptizedAt이 null인 학생 → 결과에서 제외
2. baptizedAt이 "MM/DD" 형식이 아닌 학생 → 결과에서 제외
3. 삭제/졸업 학생 → 결과에서 제외
