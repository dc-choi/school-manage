...
- **문제**:
  ...
  - 서비스가 수집하는 정보: 계정 정보(아이디, 이름, 비밀번호) + 사용자가 입력하는 학생 정보(이름, 세례명, 성별, 나이, 연락처, **보호자 연락처**, 세례일, 메모, 출석 기록)
...
- [x] 개인정보 처리방침 버전 관리 (2026-04-24 완료) — `CURRENT_PRIVACY_VERSION` 서버 상수 + `Account.privacyPolicyVersion` 컬럼. 버전 미스매치 시 `/consent` 강제 재노출
...
- [x] 수집 항목 확대(2026-04-24 "보호자 연락처" 추가)에 따른 **기존 회원 재동의** (2026-04-24 완료) — `privacyPolicyVersion` 버전 관리 도입으로 소급 재동의 장치 구축. 알림 인프라(SMS/카톡/이메일) 부재 상황에서 `/consent` 재노출이 유일한 공식 고지 경로. 259 기존 계정 모두 v1으로 세팅 → 다음 로그인 시 변경 사항 블록과 함께 재동의 UI 진입
