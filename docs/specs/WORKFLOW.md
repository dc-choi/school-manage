...
브레인스토밍(`docs/brainstorm/`)의 결정 사항은 **자동으로 SDD TARGET에 등록되지 않는다.**

| 브레인스토밍 결과 | SDD 등록 조건 |
|--------------|------------|
| 조건부 승인 (착수 조건 있음) | 착수 조건 충족 후 사업 에이전트가 로드맵에 정식 반영 → SDD 0단계에서 등록 |
| 리서치/검증 필요 | 검증 완료 후 사업 에이전트가 판단 → 로드맵 반영 → SDD 등록 |
| 기각 | 등록하지 않음 |

> **핵심 원칙**: 브레인스토밍은 리서치이지 결정이 아니다. 사용자의 명시적 결정 없이 사업 문서(로드맵, STATUS 등)에도, SDD TARGET에도 반영하지 않는다. `docs/business/6_roadmap/roadmap.md`에 정식 반영된 항목만 SDD TARGET 등록 대상이다. 사업 문서 반영 규칙은 `docs/business/WORKFLOW.md` 참조.
